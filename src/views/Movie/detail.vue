<template>
  <div id="detailContrainer" class="slide-enter-active">
    <Header title="影片详情"><i @touchstart = 'handleToBack()' class="iconfont icon-right"></i></Header>
      ...
      <div class="detail_player swiper-container" ref="detail_player">
        <ul class="swiper-wrapper">
          <li v-for="(item,index) in detailMovie.photos" :key="index" class="swiper-slide">
            <div>
              <img :src="item | setWH(335.209)" alt="">
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  mounted(){
    this.$http.get('/ajax/detailmovie?movieId='+this.movieId).then(res => {
      if (res.status == 200) {
        this.loading = false
